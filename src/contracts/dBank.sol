...
contract dBank {
  ...
  function deposit() payable public {
    ...
    require(isDeposited[msg.sender] == false, 'Eror deposit alrrady active');
  ...
  function withdraw() public {
    ...
    //assign msg.sender ether deposit balance to svariable for event
    uint userBalance = etherBalanceOf[msg.sender];

    //check user's hold time
    uint depositTime = block.timestamp - depositStart[msg.sender];

    //31668017-interest(10% APY) oer second for min. deposit amount (0.01 ETH), cuz:
    //1e15(10% of 0.01 ETH) / 31577600 (seconds in 365.25 days)

    //(etherBalanaceOf[msg.sender] / 1e16)c- calc. how much higher interest will be (based on deposit
    //for a min. deposit (0.01 ETH), (etherBalanceOf[msg.sender] / 1e16) = 1(the same, 31668017/s)
